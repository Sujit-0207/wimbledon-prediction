...
def predict(df, label, extra_ranks=None, withdrawn=None):
    ...
    # remove confirmed withdrawals
    if withdrawn:
        before = len(sc)
        sc = sc[~sc["player"].apply(name_key).isin(withdrawn)]
        removed = before - len(sc)
        if removed:
            print(f"  Withdrawn      : {removed} player(s) removed")
...
def load_withdrawals():
    """Load confirmed Wimbledon 2026 withdrawals; return {gender: set_of_name_keys}."""
    path = os.path.join(FILES2_DIR, "withdrawals_2026.csv")
    if not os.path.exists(path):
        return {"Men": set(), "Women": set()}
    wd = pd.read_csv(path)
    result = {}
    for gender, grp in wd.groupby("gender"):
        result[gender] = {name_key(n) for n in grp["name"]}
    return result
...
if __name__ == "__main__":
    ...
    print(f"Withdrawals loaded : {sum(len(v) for v in withdrawals.values())} players "
          f"({len(withdrawals.get('Men', set()))} men, {len(withdrawals.get('Women', set()))} women)")

    men_qf   = predict(men,   "Men",   withdrawn=withdrawals.get("Men",   set()))
    women_qf = predict(women, "Women", extra_ranks=extra_ranks,
                       withdrawn=withdrawals.get("Women", set()))
    ...
    # print results first so they're always visible
    print("\n" + "=" * 60)
    ...
    try:
        ...
        print(f"Could not save CSV (file open elsewhere): {OUTPUT}")
